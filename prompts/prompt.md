...
- **투자원금**: ₩{total_investment_krw:,} (실제 주식 매수 금액)
- **현재가치**: ₩{current_value_krw:,}
- **투자원금 기준 수익**: {total_return_krw:+,}원 ({total_return_percent:+.2f}%)
- **초기자금 기준 수익**: {total_return_from_initial_krw:+,}원 ({total_return_from_initial_percent:+.2f}%)
...
- **기술지표**: indicators[symbol].rsi14, indicators[symbol].ema20, indicators[symbol].ema50
  - ⚠️ **기술지표 누락 대응**: 신규 종목이나 데이터 부족 시 일부 지표가 없을 수 있음
    - RSI14만 있는 경우: RSI 기준으로만 분석 (과매수/과매도 판단)
    - EMA20만 있는 경우: 단기 추세만 분석
    - 지표가 전혀 없는 경우: 현재가와 최근 가격 변동성, 뉴스, 거래량으로 대체 분석
    - 보유 종목의 기술지표가 없으면 현재가 기준으로 손익만 계산하고 매도 판단은 보수적으로
...
- **매도 시 현금화 계산**: 현재가 × 보유주수 = 예상 현금화 금액 (정확한 금액 명시)
...
- **섹터 다양화 필수**:
  - AI/반도체만 집중하지 말고 Healthcare, Biotech, EV, Fintech 등 다양한 섹터 고려
  - 각 섹터별 최근 모멘텀과 성장 전망 분석 후 유망 섹터 선정
  - 섹터 회전 전략: 과열된 섹터는 줄이고 저평가된 유망 섹터 발굴

**구체적 매매 계획 (필수):**
⚠️ **현금 흐름 계산**: 매도 시 현재가 기준 현금화 금액을 정확히 산출하여 매수 가능 금액 제시

1. **매도 계획 (현금화 계산 포함)**: 
   - TSLA X주 매도 → 현재가 $XXX × X주 = 예상 회수금액 $X,XXX
   - GOOGL X주 매도 → 현재가 $XXX × X주 = 예상 회수금액 $X,XXX
   - 총 확보 자금: $X,XXX (매도금) + 기존 현금 $XXX = **총 가용현금 $X,XXX**

2. **신규 매수 계획 (가용현금 기준)**:
   - **총 투자 가능 금액**: $X,XXX (위 매도 후 확보된 총 현금)
   - 추천 종목 1: XXXX - 현재가 $XXX → X주 매수 ($X,XXX 투입)
   - 추천 종목 2: YYYY - 현재가 $XXX → X주 매수 ($X,XXX 투입)  
   - **잔여 현금**: $XXX (추가 기회 대기용)
...
**⚡ 이번 주 핵심 액션**:
- **매도 추천**: [종목명] N주 → $XXX 현금화 (이유: 과열/목표가 도달/손절)
- **매수 추천**: [종목명] N주 ($XXX 투입) (이유: 저가매수기회/모멘텀/성장스토리)

**💡 1000만원 목표 전망**: [낙관적/신중낙관/중립/신중비관/비관적]
- **근거**: 현재 진행률 XX%, 필요 월평균 수익률 XX%, 시장 환경 및 전략 실현 가능성 종합 판단

**📊 위험도 평가**: [안전/보통/공격적/위험] - 현재 전략의 리스크 수준
